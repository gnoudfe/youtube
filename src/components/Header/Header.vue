...
const { SearchState, actionGetSearchVideo, actionGetDetailSearchVideo } =
  useSearchStore();
const { state } = useSideBarStore();
const toggleSideBar = () => {
  state.isSidebar = !state.isSidebar;
  state.isSidebarVideo = true;
};
const handleClickOutside = (event) => {
  if (!searchInput.value.contains(event.target)) {
    if (isDropDownSearch.value) {
      isDropDownSearch.value = false;
    }
  }
};
...
const toggleDropDown = () => {
  if (SearchState.searchQuery == "") {
    ...
    isDropDownSearch.value = true;
  }
};

const Search = async () => {
  isDropDownSearch.value = true;
  await actionGetSearchVideo(SearchState.searchQuery);
};
const ToRelateVideoSearch = async (query) => {
  isDropDownSearch.value = false;
  SettingsState.isLoading = true;
  await actionGetDetailSearchVideo(query);
  SettingsState.isLoading = false;
  route.push(`search?q=${query}`);
};


const handleEnterKey = (query) => {
  ToRelateVideoSearch(query);
};
...
<template>
  <div
    class="max-h-[65px] h-[65px] bg-[#070707] fixed top-0 bottom-0 left-0 right-0 z-10"
  >
    <div
      class="w-full h-full pr-[50px] pl-[20px] flex items-center justify-between"
    >
      <div class="flex items-center">
        <div
          class="text-white cursor-pointer w-[40px] h-[40px] rounded-full relative flex items-center justify-center hover:bg-[#272727] ease-in duration-400 transition"
        >
        ...
        <div class="ml-[20px] flex items-center">
          <router-link to="/" class="block w-[90px] h-[20px]">
            <div class="relative">
              <svg
                viewBox="0 0 90 20"
              ...
              <span
                class="block absolute text-[#AAAAAA] text-[10px] font-medium top-[-5px] -right-[20px]"
                >VN</span
              >
            </div>
          </router-link>
      ...
      <div class="flex items-center">
        <div
          class="max-w-[536px] relative w-[536px] border-[#454444] border border-solid h-[40px] rounded-full flex items-center pl-[20px]"
          ref="root"
        >
          <input
            type="text"
            placeholder="Tìm kiếm"
            class="placeholder:text-[#888888] font-medium text-[#fff] w-[80%] bg-transparent h-full border-none outline-none"
            v-model="SearchState.searchQuery"
            @keyup.enter="handleEnterKey(SearchState.searchQuery)"
          ...
          <ul
            v-if="
              isDropDownSearch &&
              SearchState.searchQuery !== '' &&
              SearchState.VideoSearch.length > 0
            "
            class="absolute min-h-[300px] list-none bg-[#212121] top-[50px] rounded-lg w-full left-0 py-3 z-[9999999]"
          >
            <li
              v-for="query in SearchState.VideoSearch[1]"
              class="text-white w-full block px-[20px] cursor-pointer hover:bg-[#363636fa] transition-colors py-[6px]"
              @click="ToRelateVideoSearch(query)"
            >
              {{ query }}
            </li>
          </ul>
          <div
            class="absolute right-0 max-w-[64px] w-[64px] h-full flex items-center justify-center cursor-pointer bg-[#222222] rounded-r-full"
          >
            <svg
        ...
        <div
          class="ml-[20px] flex items-center justify-center cursor-pointer w-[40px] h-[40px] rounded-full bg-[#272727] hover:bg-[#3d3d3d] transition ease-in duration-300"
        >
      ...
      <div class="flex items-center">
        <div
          class="mr-[10px] w-[40px] h-[40px] rounded-full relative flex cursor-pointer items-center justify-center hover:bg-[#272727] ease-in duration-400 transition"
        >
          <svg
            height="24"
            style="pointer-events: none; display: block"
        ...
        <div
          class="mr-[25px] w-[40px] h-[40px] rounded-full relative flex cursor-pointer items-center justify-center hover:bg-[#272727] ease-in duration-400 transition"
        >
          <svg
            enable-background="new 0 0 24 24"
            height="24"
        ...
        <div class="w-[32px] h-[32px] rounded-full cursor-pointer">
          <img
            src="https://yt3.ggpht.com/yti/AOXPAcWTX0rdYEZfQgvMTIT09InsqfDndqCcCLW7VxvzCA=s88-c-k-c0x00ffffff-no-rj"
            alt="avatar user"
