...
<template>
  <div class="flex items-center flex-wrap mx-[-30px]">
    <div
      v-for="(item, index) in props.data"
      :key="index"
      class="w-[16.6667%] px-[30px] mb-[44px]"
    >
      <div class="w-full">
          ...
          <div>
            <div class="relative">
              <img
                :src="item?.thumbnail"
                alt="thumbnail video"
                class="rounded-full object-contain bg-[#5e5e5e] w-full min-h-[154px]"
              />
              <div>
                <div
            ...
            <div class="mt-[20px] min-h-[70px] text-center">
              <div class="ml-[10px] relative">
                ...
                <div>
                  <div class="cursor-pointer">
                    <div
                      class="text-sm text-[#AAAAAA] flex items-center mt-[3px] justify-center"
                    >
                      <span
                        class="block text-center text-white font-semibold"
                        >{{ item.name }}</span
                      >
                    </div>
                    <div
                      class="text-sm text-[#AAAAAA] flex items-center mt-[3px] justify-center"
                    >
                      <span class="block text-center text-[12px]"
                        >{{ formatViews(item.subscribers) }} Người đăng ký</span
                      >
                    </div>
                  </div>
                </div>
